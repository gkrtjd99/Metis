...
Do not expand authority or scope. If the host lacks nested-agent support, state that limitation; adding an Agent tool alone is not evidence. When the configured host is explicitly opted into `mode: "host-relay"` with `childSpawning: true` and verified evidence, claim and prepare the approved batch, then relay only its batch ID to the top-level Main host instead of performing a native nested spawn. Main uses authenticated `metis relay read <batch-id>` to obtain descriptors; relay read does not spawn. Existing host tools create children and return real receipts for `schedule ack`; keep controller credentials with Main, while the owner retains child decisions and completion.
